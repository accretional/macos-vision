...
if [ -f "$got" ]; then
    ...
    jq -e '.classifications[0].identifier' "$got" >/dev/null 2>&1 && pass "classify: identifier field present" || fail "classify: identifier field missing"
else
...
"$BINARY" classify --img "$IMG/sad_pablo.png" --operation horizon --output "$TMP" 2>/dev/null || true
...
if [ -f "$got" ]; then
    ...
    jq -e '.horizon' "$got" >/dev/null 2>&1 && pass "horizon: horizon field present" || fail "horizon: horizon field missing"
else
...
if [ -f "$got" ]; then
    ...
    jq -e '.contourCount' "$got" >/dev/null 2>&1 && pass "contours: contourCount field present" || fail "contours: contourCount field missing"
    count=$(jq '.contourCount' "$got" 2>/dev/null || echo 0)
...
if [ -f "$got" ]; then
    ...
    jq -e '.featurePrint.elementCount' "$got" >/dev/null 2>&1 && pass "feature-print: elementCount present" || fail "feature-print: elementCount missing"
    jq -e '.featurePrint.data' "$got" >/dev/null 2>&1 && pass "feature-print: data present" || fail "feature-print: data missing"
else
...
echo "$err" | grep -qi "img\|must be provided\|error" && pass "classify: missing input error shown" || fail "classify: no error on missing input"
err=$("$BINARY" classify --img "$img" --operation bad-op 2>&1 || true)
