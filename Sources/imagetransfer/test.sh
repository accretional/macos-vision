...
# ── error handling ────────────────────────────────────────────────────────────
echo "── imagetransfer: error handling ─────────────────────────────────────────────"

err=$("$BINARY" imagetransfer --operation bad-op 2>&1 || true)
...
for op in list-files "camera/files" camera/thumbnail camera/metadata camera/import camera/delete camera/capture camera/sync-clock; do
    err=$("$BINARY" imagetransfer --operation "$op" 2>&1 || true)
...
echo "── imagetransfer: camera/import requires --output ────────────────────────────"
err=$("$BINARY" imagetransfer --operation camera/import 2>&1 || true)
...
if [ -f "$TMP/devices.json" ]; then
    ...
    jq -e '.subcommand == "imagetransfer"' "$TMP/devices.json" >/dev/null 2>&1 \
