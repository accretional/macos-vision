...
if [ -f "$got" ]; then
    ...
    count=$(jq '.result.faces | length' "$got" 2>/dev/null || echo 0)
    [ "${count:-0}" -gt 0 ] && pass "face-rectangles: $count face(s) detected" || fail "face-rectangles: no faces detected"
    jq -e '.result.faces[0].boundingBox.x' "$got" >/dev/null 2>&1 && pass "face-rectangles: boundingBox present" || fail "face-rectangles: boundingBox missing"
else
...
if [ -f "$got" ]; then
    ...
    count=$(jq '.result.faces[0].landmarks | length' "$got" 2>/dev/null || echo 0)
...
if [ -f "$got" ]; then
    ...
    jq -e '.result.faces[0].quality' "$got" >/dev/null 2>&1 && pass "face-quality: quality field present" || fail "face-quality: quality field missing"
else
...
if [ -f "$got" ]; then
    ...
    jq -e '.result.bodies' "$got" >/dev/null 2>&1 && pass "body-pose: bodies field present" || fail "body-pose: bodies field missing"
else
...
if [ -f "$got" ]; then
    ...
    jq -e '.result.humans' "$got" >/dev/null 2>&1 && pass "human-rectangles: humans field present" || fail "human-rectangles: humans field missing"
else
