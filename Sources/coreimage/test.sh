...
err=$("$BINARY" coreimage --input "$IMG" --operation apply-filter --output "$TMP/x.json" --no-stream 2>&1 || true)
echo "$err" | grep -qi "filter\|--filter-name\|error" \
...
err=$("$BINARY" coreimage --input "$IMG" --operation apply-filter --filter-name NotARealFilter \
      --output "$TMP/x.json" --no-stream 2>&1 || true)
...
err=$("$BINARY" coreimage --input "$IMG" --operation apply-filter --filter-name CISepiaTone \
      --filter-params 'not valid json' --output "$TMP/x.json" --no-stream 2>&1 || true)
...
"$BINARY" coreimage --operation list-filters --output "$TMP/filters.json" --no-stream
...
"$BINARY" coreimage --operation list-filters --category-only --output "$TMP/categories.json" --no-stream
...
if $HAS_IMG; then
    ...
    "$BINARY" coreimage --input "$IMG" --operation apply-filter --filter-name CISepiaTone \
              --artifacts-dir "$TMP" --json-output "$TMP/sepia.json" --no-stream
    ...
    "$BINARY" coreimage --input "$IMG" --operation apply-filter --filter-name CIGaussianBlur \
              ...
              --artifacts-dir "$TMP" --json-output "$TMP/blur.json" --no-stream
    ...
    "$BINARY" coreimage --input "$IMG" --operation apply-filter --filter-name CIColorInvert \
              --debug --artifacts-dir "$TMP" --json-output "$TMP/invert.json" --no-stream
    ...
    "$BINARY" coreimage --input "$IMG" --operation apply-filter --filter-name CISepiaTone \
              --output "$TMP/exact_output.png" --json-output "$TMP/exact.json" --no-stream
...
err=$("$BINARY" coreimage --operation apply-filter --filter-name CISepiaTone \
      --format notaformat --output "$TMP/x.json" --no-stream 2>&1 || true)
echo "$err" | grep -qi "format\|unknown\|error" \
    && pass "--format: invalid format rejected" || fail "--format: invalid format not rejected"

if $HAS_IMG; then
    "$BINARY" coreimage --input "$IMG" --operation apply-filter --filter-name CISepiaTone \
              --format jpg --artifacts-dir "$TMP" --json-output "$TMP/sepia_jpg.json" --no-stream
    ...
    "$BINARY" coreimage --input "$IMG" --operation apply-filter --filter-name CISepiaTone \
              --format tiff --artifacts-dir "$TMP" --json-output "$TMP/sepia_tiff.json" --no-stream
...
err=$("$BINARY" coreimage --operation auto-adjust --output "$TMP/x.json" --no-stream 2>&1 || true)
...
if $HAS_IMG; then
    "$BINARY" coreimage --input "$IMG" --operation auto-adjust \
              --artifacts-dir "$TMP" --json-output "$TMP/autoadj.json" --no-stream
    ...
    "$BINARY" coreimage --input "$IMG" --operation auto-adjust \
              --format jpg --artifacts-dir "$TMP" --json-output "$TMP/autoadj_jpg.json" --no-stream
    ...
    "$BINARY" coreimage --input "$IMG" --operation auto-adjust \
              --debug --artifacts-dir "$TMP" --json-output "$TMP/autoadj_debug.json" --no-stream
