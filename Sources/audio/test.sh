...
echo "$err" | grep -qi "audio\|--audio\|mic\|screenshot\|camera\|record\|provide\|error" \
    && pass "audio: missing input error shown" || fail "audio: no error on missing input"

err=$("$BINARY" audio --audio "$AUDIO" --operation not-a-real-op --output "$TMP/x.json" 2>&1 || true)
echo "$err" | grep -qi "unknown\|valid\|error" \
    && pass "audio: unknown operation rejected" || fail "audio: unknown operation not rejected"

err=$("$BINARY" audio --audio /nonexistent/no_such_file.wav --operation classify --output "$TMP/x.json" 2>&1 || true)
...
echo "── audio: classify ─────────────────────────────────────────────────────────"
if $has_audio; then
    "$BINARY" audio --audio "$AUDIO" --operation classify --output "$TMP/classify.json"
    ...
    if [ -f "$got" ]; then
        ...
        jq -e '.results | type == "array"' "$got" >/dev/null 2>&1 && pass "classify: results array" || fail "classify: results not an array"
    else
...
echo "── audio: noise ────────────────────────────────────────────────────────────"
if $has_audio; then
    "$BINARY" audio --audio "$AUDIO" --operation noise --output "$TMP/noise.json"
    got="$TMP/noise.json"
    if [ -f "$got" ]; then
        pass "noise: output produced"
        jq empty "$got" 2>/dev/null && pass "noise: valid JSON" || fail "noise: invalid JSON"
        jq -e '.results | type == "array"' "$got" >/dev/null 2>&1 && pass "noise: results array" || fail "noise: results not an array"
        n=$(jq '.results | length' "$got" 2>/dev/null || echo 0)
...
echo "── audio: pitch ────────────────────────────────────────────────────────────"
if $has_audio; then
    "$BINARY" audio --audio "$AUDIO" --operation pitch --output "$TMP/pitch.json"
    got="$TMP/pitch.json"
    if [ -f "$got" ]; then
        pass "pitch: output produced"
        jq empty "$got" 2>/dev/null && pass "pitch: valid JSON" || fail "pitch: invalid JSON"
        jq -e '.results | type == "array"' "$got" >/dev/null 2>&1 && pass "pitch: results array" || fail "pitch: results not an array"
    else
...
echo "── audio: detect ─────────────────────────────────────────────────────────────"
if $has_audio; then
    "$BINARY" audio --audio "$AUDIO" --operation detect --output "$TMP/detect.json"
    got="$TMP/detect.json"
    if [ -f "$got" ]; then
        pass "detect: output produced"
        jq empty "$got" 2>/dev/null && pass "detect: valid JSON" || fail "detect: invalid JSON"
        jq -e '.results | type == "array"' "$got" >/dev/null 2>&1 && pass "detect: results array" || fail "detect: results not an array"
    else
...
echo "── audio: shazam ─────────────────────────────────────────────────────────────"
if $has_audio; then
    "$BINARY" audio --audio "$AUDIO" --operation shazam --output "$TMP/shazam.json"
    got="$TMP/shazam.json"
    if [ -f "$got" ]; then
        pass "shazam: output produced"
        jq empty "$got" 2>/dev/null && pass "shazam: valid JSON" || fail "shazam: invalid JSON"
        jq -e '.results | type == "object"' "$got" >/dev/null 2>&1 && pass "shazam: results object" || fail "shazam: results not an object"
        jq -e '.results | has("matched")' "$got" >/dev/null 2>&1 && pass "shazam: matched field present" || fail "shazam: matched field missing"
    else
...
echo "── audio: isolate ──────────────────────────────────────────────────────────"
if $has_audio; then
    "$BINARY" audio --audio "$AUDIO" --operation isolate --output "$TMP/isolate.json"
    ...
    if [ -f "$got" ]; then
        ...
        outpath=$(jq -r '.results.output // empty' "$got" 2>/dev/null || true)
...
# ── batch + merge (noise) ───────────────────────────────────────────────────
echo "── audio: batch --merge ─────────────────────────────────────────────────────"
if $has_audio; then
    mkdir -p "$TMP/batch_in" "$TMP/batch_out"
    cp "$AUDIO" "$TMP/batch_in/clip.aiff"
    "$BINARY" audio --audio-dir "$TMP/batch_in" --operation noise --output-dir "$TMP/batch_out" \
        --merge --output "$TMP/merged_noise.json"
    if [ -f "$TMP/batch_out/clip.json" ] && [ -f "$TMP/merged_noise.json" ]; then
        pass "batch: per-file and merged JSON written"
        jq -e '.files | type == "array"' "$TMP/merged_noise.json" >/dev/null 2>&1 \
            && pass "merge: files array present" || fail "merge: files array missing"
    else
        fail "batch: expected clip.json and merged_noise.json"
    fi
else
    pass "batch: skipped (say could not create sample audio)"
fi
echo
...
if $has_audio; then
    set +e
    tout=$("$BINARY" audio --audio "$AUDIO" --operation transcribe --output "$TMP/tr.json" 2>&1)
    tec=$?
    set -e
    if [ "$tec" -eq 0 ] && [ -f "$TMP/tr.json" ]; then
        ...
        jq -e '.results | type == "array"' "$TMP/tr.json" >/dev/null 2>&1 \
